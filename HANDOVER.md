...
## Project-specific: Minify++ Conformance

This repository is a standards-conformance lab, not the Minify++ implementation.
Keep Minify++ product code and its fast unit/regression suite in the sibling
`minify` repository. This repository owns heavyweight independent-corpus
acquisition, extraction/adapters, browser/runtime oracles, historical result
artifacts, and the static conformance dashboard.

Do not vendor WPT or Test262 into git. `tools/conformance.py sync` checks them out
under ignored `.state/upstreams/` and records the exact revision used. A completed
